...
<?php
	...
	if (isset($_GET['code'])) { // if the code param has been sent to this page... we are in Step 2
		...
		$_SESSION['token'] = $haravanClient->getAccessToken($_GET['code']);
		if ($_SESSION['token'] != '')
	...
	// if they posted the form with the shop name
		...
		header("Location: " . $haravanClient->getAuthorizeUrl(SHOPIFY_SCOPE, $pageURL));
		exit;
	}
...
<?php
    ...
    $sc = new HaravanClient($_GET['shop'], '', SHOPIFY_API_KEY, SHOPIFY_SECRET);
